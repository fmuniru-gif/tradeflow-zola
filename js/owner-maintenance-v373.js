/* ZEZMS Owner Edition v3.7.3 - account filters and purchase orders */
(function () {
  'use strict';
  ...
  function orderLinesHTML() {
    return purchaseOrderDraft.lines.map(function (line, index) {
      return '<tr><td class="mono">' + esc(line.productId || '-') + '</td><td>' + esc(line.product) + '</td>'
        ...
        + '<td class="right mono">' + fmtN(line.total) + '</td>'
        + '<td><button class="btn sm danger" onclick="removePurchaseOrderLine(' + index + ')">Remove</button></td></tr>';
    }).join('') || '<tr><td colspan="6" class="empty">No products added to this purchase order.</td></tr>';
  }
  ...
  function viewPurchaseOrders() {
    ...
    return editBanner + '<div class="grid g2"><div>'
      ...
      + '<thead><tr><th>Product ID</th><th>Product</th><th class="right">Qty</th><th class="right">Unit cost</th><th class="right">Line total</th><th></th></tr></thead>'
  ...
  window.purchaseOrderProductChanged = function () {
    ...
    if (cost) cost.value = latest ? (Number(latest.uCost) || 0) : (Number(product.costPrice) || 0);
  };
  ...
  window.saveNewPurchaseOrderProduct = function () {
    ...
    DB.products.push({ id: productId, name: name, category: category, costPrice: unitCost, uPrice: unitPrice });
    purchaseOrderProductSelection = productId;
    saveDB();
    closeModal();
    render();
    setTimeout(function () { purchaseOrderProductChanged(); }, 0);
    toast('Product saved with cost and selling prices, then selected - ' + productId + '.');
  };
  ...
  function purchaseOrderPaperHTML(order) {
    ...
    const rows = (order.lines || []).map(function (line, index) {
      return '<tr><td style="text-align:center">' + (index + 1) + '</td><td class="mono">' + esc(line.productId || '-') + '</td>'
        ...
        + '<td style="text-align:right">' + fmtN(line.unitCost) + '</td><td style="text-align:right">' + fmtN(line.total) + '</td></tr>';
    }).join('');
    return '<div class="document-paper purchase-order-paper"><div class="doc-head"><div><div style="font-size:20px;font-weight:900">' + esc(biz.name) + '</div>'
      ...
      + '<table><thead><tr><th>#</th><th>Product ID</th><th>Product</th><th style="text-align:right">Qty</th><th style="text-align:right">Unit cost</th><th style="text-align:right">Total</th></tr></thead><tbody>' + rows + '</tbody></table>'
