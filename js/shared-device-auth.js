/* ZEZMS v3.7.3 — Cross-Device Staff Access */
(function(){
'use strict';
...
const VIEW={dashboard:'VIEW_DASHBOARD',pos:'SALE_OUT',receipts:'VIEW_RECEIPTS',stockin:'STOCK_IN',products:'VIEW_PRODUCTS',stock:'VIEW_STOCK',cash:'VIEW_CASH',expenses:'VIEW_EXPENSES',accounts:'VIEW_ACCOUNTS',reports:'VIEW_REPORTS',sync:'MANAGE_SYNC',integrity:'MANAGE_SYNC',settings:'MANAGE_SETTINGS'};
...
function deviceArgs(){const s=cloud();return{p_device_id:String(s.deviceId||''),p_device_name:String(s.deviceName||'ZEZMS Device'),p_platform:String(navigator.userAgent||'').slice(0,240),p_app_version:typeof APP_VERSION!=='undefined'?String(APP_VERSION):'3.7.3'};}
...
window.sharedDeviceConfirmRefreshUsers=function(){
  if(!window.confirm('Refresh the staff list from Cloud Sync? This can replace this device’s local staff list with the published version.'))return false;
  return window.sharedDeviceRefreshUsers(true);
};
...
window.sharedDeviceConfirmOwnerRecovery=function(){
  if(!window.confirm('Owner recovery / first setup can replace shared-device staff access. Continue only if you intend to recover access or perform the first setup.'))return false;
  return window.sharedDeviceOwnerRecovery();
};
