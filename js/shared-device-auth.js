...
async function deleteUser(id){try{if(!can('MANAGE_STAFF'))throw new Error('Only Owner or Admin can delete staff.');const m=model(),u=m.users.find(x=>x.id===id);if(!u)throw new Error('Staff account not found.');if(u.role==='OWNER')throw new Error('The OWNER account cannot be deleted.');if(session.sharedUserId===u.id)throw new Error('You cannot delete the account currently signed in.');if(role()==='ADMIN'&&u.role==='ADMIN')throw new Error('Only the OWNER can delete another ADMIN.');if(!window.confirm(`Permanently delete ${u.name}?\n\nTheir historical transactions will remain, but the staff account and password will be removed from every synced device.`))return;if(!await reauth('Confirm permanent staff deletion'))return;const index=m.users.findIndex(x=>x.id===id);if(index<0)throw new Error('Staff account not found.');const deleted=m.users.splice(index,1)[0];m.updatedAt=stamp();writeDirectoryRoot();audit('STAFF_ACCOUNT_DELETED',{userId:deleted.id,name:deleted.name,role:deleted.role});saveDB();try{await publishDirectory(false);}catch(error){console.warn('Deleted staff could not be published immediately',error);}closeModal();render();refreshUsers();toast(`Staff deleted: ${deleted.name}`);}catch(e){toast(e.message||String(e),'err');}}
...
window.sharedDeviceDeleteUser=function(id){ deleteUser(id); };
